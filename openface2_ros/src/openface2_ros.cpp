...
public:
        ...
        , face_analyser(face_analysis_params)
    {
        ...
        if (!pnh.getParam("image_topic", image_topic_))
            throw invalid_argument("Expected ~image_topic parameter");

        const auto base_path = package::getPath("openface2_ros");

        pnh.param<bool>("publish_viz", publish_viz_, false);

        if (!pnh.getParam("max_faces", max_faces_))
            pnh.param<int>("max_faces", max_faces_, 1);
        if (max_faces_ <= 0)
            throw invalid_argument("~max_faces must be > 0");

        float rate = 0;
        if (!pnh.getParam("rate", rate))
            pnh.param<float>("rate", rate, 4.0);
        if (rate <= 0)
        ...
        head_pose_pub_ = nh_.advertise<geometry_msgs::PoseStamped>("openface2/head_pose", 10);
        if (publish_viz_)
...
private:
    ...
    void process_incoming_(const sensor_msgs::ImageConstPtr& img, const sensor_msgs::CameraInfoConstPtr& cam)
    {
        ...
        if (cx == 0)
            cx = cv_ptr_rgb->image.cols / 2.0;
        if (cy == 0)
            cy = cv_ptr_rgb->image.rows / 2.0;
            ...
            if (!active_models[model]) {
                for (size_t detection_ind = 0; detection_ind < face_detections.size(); ++detection_ind) {
                    ...
                    if (face_detections_used[detection_ind].compare_and_swap(true, false) == false) {
                        ...
                        active_models[model] = true; // break out of the loop as the tracker has been reinitialised
                        break;
                    }
                }
            } else {
        ...
        if (publish_viz_)
            visualizer.SetImage(viz_img, fx, fy, cx, cy);
        ...
        for (size_t model = 0; model < face_models.size(); ++model) {
            ...
            if (active_models[model]) {
                ...
                for (const auto& au : aus)
                    face.action_units.push_back(get<1>(au));
                ...
                for (auto it = aus_reg.begin(); it != aus_reg.end(); it++) {
                    if (it->first.compare("AU02") == 0 && it->second > 2.0) {
                        face.head_pose.position.z = transform.transform.translation.z;
                        geometry_msgs::PoseStamped pose;
                        pose.header.stamp = ros::Time::now();
                        pose.header.frame_id = out.str();
                        pose.pose = face.head_pose;
                        head_pose_pub_.publish(pose);
                    }
                }
            }
        }
